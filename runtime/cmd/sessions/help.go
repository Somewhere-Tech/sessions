...
var commandTable = []commandSpec{
	...
	},
	{
		...
		longHelp: "Continue an exact conversation returned by Sessions history. The authenticated history id supplies provider identity and workspace. Claude resumes in its native interactive Remote Control runtime by default; Codex resumes in its Rich app-server runtime. Claude prompt-index-only records use the provider's native resume command from that recorded workspace; Sessions never guesses another folder or conversation. --structured explicitly selects headless Rich Claude when automation requires it. --terminal explicitly selects the original provider's terminal interface and cannot be combined with a cross-provider continuation. --remote-control remains a compatibility flag for an explicit Terminal Claude continuation. --source links an ended Sessions runtime, and --repair only completes missing records for an already-live successor.",
