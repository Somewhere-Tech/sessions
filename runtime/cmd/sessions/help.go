...
var commandTable = []commandSpec{
	...
	{
		name: "continue", usage: "continue <history-id> [--with claude|codex] [--terminal [--remote-control]] [--force] [--source SESSION] [--repair LIVE-SUCCESSOR]",
		summary: "continue one exact saved conversation", group: dailyCommandGroup, localJSON: true,
		longHelp: "Continue an exact conversation returned by Sessions history. The authenticated history id supplies provider identity and workspace. Claude prompt-index-only records use the provider's native resume command from that recorded workspace; Sessions never guesses another folder or conversation. Rich is the default. --terminal explicitly opens the original provider's terminal interface and cannot be combined with a cross-provider continuation. --remote-control starts that Terminal Claude continuation with Claude Remote Control enabled. --source links an ended Sessions runtime, and --repair only completes missing records for an already-live successor.",
