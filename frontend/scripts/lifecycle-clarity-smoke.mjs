...
assert.match(navigator, /DisclosureChevron open=\{runningOpen\} \/> In focus/);
assert.match(navigator, /DisclosureChevron open=\{setAsideOpen\} \/> Later/);
assert.match(navigator, /DisclosureChevron open=\{endedOpen\} \/> Quiet/);
assert.match(navigator, /session-tree-toggle/);
assert.match(navigator, /All quiet sessions →/);
...
assert.match(navigator, /Close tab <small>keeps running in In focus<\/small>/);
assert.match(navigator, /Move to Later <small>keeps running<\/small>/);
...
const agentStep = newSession.indexOf('Who should work on this?');
const machineStep = newSession.indexOf('Where should it run?');
const folderStep = newSession.indexOf('Choose the project');
assert.ok(agentStep > 0 && agentStep < machineStep && machineStep < folderStep, 'new-session steps must be Agent, Machine, Folder');
assert.match(newSession, /Somewhere projects and recent work/);
assert.match(newSession, /Give this session its own Git copy/);
assert.match(newSession, /Sessions never stops or queues existing work automatically/);
