...
assert.match(resumeDialog, /preferredRemoteControl/);
assert.match(resumeDialog, /destinationProvider === 'claude' && runtimeMode === 'terminal' && remoteControl/);
assert.match(resumeDialog, /Keep this same Claude conversation available from claude\.ai and mobile/);
...
assert.match(newSession, /claude\.ai, and mobile stay connected to that same session/);
...
assert.match(settingsView, /On for new Claude sessions/);
assert.match(settingsView, /Local only/);
