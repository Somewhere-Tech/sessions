...
assert.match(resumeDialog, /Start Claude with access from claude\.ai and mobile/);
...
assert.match(newSession, /runtimeMode === 'rich'[\s\S]*remoteControl: 'off'/);
assert.match(newSession, /Choose Terminal for Claude slash commands and Remote Control/);
