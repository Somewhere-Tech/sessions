  ...
  const handle = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (path === '/api/retention/archive' && method === 'POST') {
      const ids = Array.isArray((body as { ids?: unknown })?.ids)
        ...
        : [];
      // runtime/internal/session/retention.go ArchiveClosed, reasons verbatim,
      // including hasRetainedDescendant (retention.go:236) — an ancestor whose
      // descendant record still exists and is not itself in this batch is
      // refused, even though archiving only hides a row and leaves the
      // append-only lineage intact. Modelled faithfully on purpose: a fake that
      // is more permissive than the daemon turns a red capability green.
      const batch = new Set(ids);
      const hasRetainedDescendant = (ancestor: string): boolean =>
        machine.sessions.some((candidate) => {
          if (candidate.id === ancestor || batch.has(candidate.id)) return false;
          let current: SessionInfo | undefined = candidate;
          const seen = new Set<string>();
          while (current?.parentSessionId) {
            if (current.parentSessionId === ancestor) return true;
            if (seen.has(current.parentSessionId)) break;
            seen.add(current.parentSessionId);
            current = machine.sessions.find((s) => s.id === current!.parentSessionId);
          }
          return false;
        });
      const items = ids.map((id) => {
        ...
        if (hasRetainedDescendant(id)) {
          return { id, name: record.name, status: 'skipped' as const, reason: 'has a retained descendant' };
        }
