...
streams one exact source conversation. `sessions docs` renders the complete offline Markdown reference,
...
`sessions resurrect` is an alias for exact continuation. No transcript is
materialized into a shared plaintext directory for OS-level grep.
